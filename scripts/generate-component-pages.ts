...
const PAGES_OUTPUT_DIR = path.join(process.cwd(), 'src/app/components/standalone');
...
function createPageTemplate(componentName: string) {
...
function generateComponentPages() {
  ...
  fs.writeFileSync(path.join(componentsDir, 'Video.tsx'), VIDEO_COMPONENT);
  ...
  files.forEach(file => {
    ...
    const pageDir = path.join(PAGES_OUTPUT_DIR, componentName);
